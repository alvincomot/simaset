      ...
      <div>
        <div className="flex items-center gap-2 px-2 py-4 mb-6 border-b border-slate-800">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">UKSW Genap 2026</p>
          </div>
        </div>
        ...
        <nav className="space-y-1">
          {filteredMenu.map((item, index) => (
            <button
              key={index}
              onClick={() => onNavigate(item.name)} // <--- Pindah halaman saat menu diklik
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition font-medium ${
                item.name === currentPage // <--- JIKA NAMA MENU SAMA DENGAN HALAMAN AKTIF
                  ? "bg-blue-600 text-white" // Kasi warna biru aktif
                  : "text-slate-300 hover:bg-slate-800 hover:text-white" // Warna biasa
              }`}
            >
              {item.icon}
              {item.name}
